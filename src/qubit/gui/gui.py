...
class QuantumGate(QComboBox):
        ...
        if self.objectName():
            ...
            if self.is_control:
                ...
                for _,x,y,to in self.parent().control_draw_list:
                    if(x==int(self.objectName()[4]) and y ==int(self.objectName()[6]) ):
            ...
            # IF X,Y,Z,H gate is choosen
                ...
                if selected_item=="X":
                    self.parent().QC.add_gate(int(self.objectName()[4]),int(self.objectName()[6]), qg.X(QUBIT_NUM,int(self.objectName()[4])))
                elif selected_item=="Y":
                    self.parent().QC.add_gate(int(self.objectName()[4]),int(self.objectName()[6]), qg.Y(QUBIT_NUM,int(self.objectName()[4])))
                elif selected_item=="Z":
                    self.parent().QC.add_gate(int(self.objectName()[4]),int(self.objectName()[6]), qg.Z(QUBIT_NUM,int(self.objectName()[4])))
                elif selected_item=="H":
                    self.parent().QC.add_gate(int(self.objectName()[4]),int(self.objectName()[6]), qg.H(QUBIT_NUM,int(self.objectName()[4])))
            else:
                self.parent().QC.del_gate(int(self.objectName()[4]),int(self.objectName()[6]))
    ...
    def handle_control(self):
        ...
        dialog.exec_()
        self.parent().control_draw_list.append([dialog.return_value1,int(self.objectName()[4]),int(self.objectName()[6]),int(dialog.return_value2)])
        self.is_control = True
        if dialog.return_value1 =="X":
            self.parent().QC.add_gate(int(dialog.return_value2),int(self.objectName()[6]),qg.X(QUBIT_NUM,int(dialog.return_value2),int(self.objectName()[4])))
        elif dialog.return_value1 =="Y":
            self.parent().QC.add_gate(int(dialog.return_value2),int(self.objectName()[6]),qg.Y(QUBIT_NUM,int(dialog.return_value2),int(self.objectName()[4])))
        elif dialog.return_value1 =="Z":
            self.parent().QC.add_gate(int(dialog.return_value2),int(self.objectName()[6]),qg.Z(QUBIT_NUM,int(dialog.return_value2),int(self.objectName()[4])))
        elif dialog.return_value1 =="H":
            self.parent().QC.add_gate(int(dialog.return_value2),int(self.objectName()[6]),qg.H(QUBIT_NUM,int(dialog.return_value2),int(self.objectName()[4])))
    ...
    def handle_selection(self, index):
        ...
        if self.objectName():
            if selected_item not in ['0' ,'1']:
                print("none")
                return None
            self.parent().qubit_update(int(selected_item),int(self.objectName()[6]))
